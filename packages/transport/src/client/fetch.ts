...
async function send<T>(method: string, path: string, body: unknown, config: ApiClientConfig = {}): Promise<T> {
  const { timeoutMs, retry: retryOptions, fetcher = fetch, onRequest, onResponse, onError, ...requestConfig } = config;
  ...
  try {
    const response = retryOptions ? await retry(attempt, retryOptions) : await attempt();
...
export const api = {
  get: <T = unknown>(path: string, config?: ApiClientConfig): Promise<T> => send<T>('GET', path, undefined, config),
  post: <T = unknown>(path: string, body?: unknown, config?: ApiClientConfig): Promise<T> => send<T>('POST', path, body, config),
  put: <T = unknown>(path: string, body?: unknown, config?: ApiClientConfig): Promise<T> => send<T>('PUT', path, body, config),
  patch: <T = unknown>(path: string, body?: unknown, config?: ApiClientConfig): Promise<T> => send<T>('PATCH', path, body, config),
  delete: <T = unknown>(path: string, config?: ApiClientConfig): Promise<T> => send<T>('DELETE', path, undefined, config),
};
