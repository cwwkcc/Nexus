...
export async function normalizeError(source: unknown): Promise<ApiError> {
  ...
  if (source instanceof Response) {
    ...
    return new ApiError({ status, code: codeFromStatus(status), message, details });
  }

  if (source instanceof DOMException && source.name === 'AbortError') {
    return new ApiError({ status: 0, code: 'TIMEOUT', message: 'Request timed out', cause: source });
  }

  if (source instanceof TypeError) {
    return new ApiError({ status: 0, code: 'NETWORK_ERROR', message: source.message || 'Network request failed', cause: source });
  }

  if (source instanceof Error) {
    return new ApiError({ status: 0, code: 'UNKNOWN', message: source.message, cause: source });
  }

  return new ApiError({ status: 0, code: 'UNKNOWN', message: 'Unknown error', cause: source });
}
