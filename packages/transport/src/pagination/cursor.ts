...
export function parseCursorParams(query: Record<string, unknown>, defaultLimit = 20): CursorPageParams {
  const cursor = typeof query.cursor === 'string' ? query.cursor : undefined;
  const rawLimit = query.limit;
  const limit = typeof rawLimit === 'string' || typeof rawLimit === 'number' ? Number(rawLimit) : defaultLimit;

  return { cursor, limit: Number.isFinite(limit) && limit > 0 ? limit : defaultLimit };
}
