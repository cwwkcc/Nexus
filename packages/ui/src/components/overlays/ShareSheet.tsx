...
export function ShareSheet({ isOpen, onClose, title, url, text, platforms = ['whatsapp', 'facebook', 'copy'], labels }: ShareSheetProps) {
  const resolvedLabels = { ...DEFAULT_LABELS, ...labels };
  const [copied, setCopied] = useState(false);
  const shareBody = text ?? title;

  const shareViaWhatsApp = () => {
    window.open(`https://wa.me/?text=${encodeURIComponent(`${shareBody}\n${url}`)}`, '_blank');
  };
  ...
  return createPortal(
    <>
      <div className="fixed inset-0 bg-overlay-heavy z-[450]" onClick={onClose} />
        ...
        <div className="p-6">
          ...
          <div className="flex gap-4 justify-around mb-6">
            {platforms.includes('whatsapp') && (
              <button onClick={shareViaWhatsApp} className="flex flex-col items-center gap-2 text-green-base hover:text-gold-base transition-colors">
                ...
                <span className="font-body text-caption">{resolvedLabels.whatsapp}</span>
              </button>
            )}
            {platforms.includes('facebook') && (
              <button onClick={shareViaFacebook} className="flex flex-col items-center gap-2 text-blue-700 hover:text-gold-base transition-colors">
                <svg width="32" height="32" viewBox="0 0 24 24" fill="currentColor">
                ...
                <span className="font-body text-caption">{resolvedLabels.facebook}</span>
              </button>
            )}
            {platforms.includes('copy') && (
              <button onClick={copyLink} className="flex flex-col items-center gap-2 text-gold-base hover:text-gold-hover transition-colors">
                <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                  <path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71" />
                  <path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71" />
                </svg>
                <span className="font-body text-caption">{copied ? resolvedLabels.copied : resolvedLabels.copyLink}</span>
              </button>
            )}
          </div>
