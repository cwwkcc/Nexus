...
export function PanoramicFacilityViewer({ images, defaultIndex = 0, className, dragHintLabel = 'Drag to explore', prevLabel = 'Previous', nextLabel = 'Next' }: PanoramicFacilityViewerProps) {
  ...
  return (
    <div className={cn('relative overflow-hidden rounded-lg', className)}>
      <div ref={containerRef} className="relative aspect-[16/9] bg-surface-deep cursor-grab active:cursor-grabbing overflow-hidden" onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp}>
        <div className="relative w-[120%] h-full transition-transform duration-75 ease-out" style={{ transform: `translateX(${panX}px)` }}>
          <Image src={currentImage.src} alt={currentImage.alt} fill className="object-cover" draggable={false} />
        </div>
      ...
      {totalImages > 1 && (
        <div className="flex gap-space-2 mt-space-3 overflow-x-auto pb-space-2">
          {images.map((img, idx) => (
            <button key={idx} onClick={() => setCurrentIndex(idx)} className={cn('relative w-20 h-12 flex-shrink-0 rounded-md overflow-hidden transition-all', idx === currentIndex && 'ring-2 ring-gold-base')}>
