...
export function SectionErrorBoundary({ children, sectionName, fallback, message, retryLabel = 'Try Again' }: SectionErrorBoundaryProps) {
      ...
      fallback ?? (
        <section className="py-space-12 px-space-4">
          <div className="max-w-2xl mx-auto text-center">
            <p className="font-body text-body-sm text-text-muted">{message ?? `The ${sectionName} section encountered an error. Please try refreshing the page.`}</p>
            <button onClick={resetError} className="mt-space-4 px-space-4 py-space-2 bg-green-base text-text-inverse font-body text-caption uppercase tracking-caption hover:bg-green-hover transition-colors">
              {retryLabel}
            </button>
          </div>
        </section>
