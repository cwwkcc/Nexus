  ...
  if (successMessage) {
    return (
      <div role="status" aria-live="polite" className={cn('flex items-start gap-space-3 p-space-5', 'bg-semantic-success-surface border border-semantic-success-base', 'border-l-[3px] border-l-semantic-success-base')}>
        <span aria-hidden="true" className="text-semantic-success-base text-base leading-none mt-0.5">
          ✓
        </span>
        <p className="font-body text-body-sm text-semantic-success-base leading-relaxed">{successMessage}</p>
      </div>
    );
  }
