...
export function buildAlumniSubmitSchema(messages?: { name?: string; graduationYear?: string; quote?: string }) {
  return z.object({
...
export type AlumniSubmitFormValues = z.infer<ReturnType<typeof buildAlumniSubmitSchema>>;
...
export interface AlumniSubmitFormProps {
   ...
   * `@nexus/contracts` directly within this component so the value/label
...
const DEFAULT_LABELS: Required<AlumniSubmitFormLabels> = {
  ...
  successMessage: 'Thank you! Your profile has been submitted and will appear once reviewed.',
  errorMessage: 'Failed to submit. Please try again later.',
};

export function AlumniSubmitForm({ onSubmit, streamOptions, labels, schema, className }: AlumniSubmitFormProps) {
  ...
  const streamSelectOptions = [{ value: '', label: resolvedLabels.streamUnknown }, ...streamOptions];

  return (
    <form onSubmit={handleSubmit(handleFormSubmit)} className={className ?? 'flex flex-col gap-space-6'}>
      <FormFieldGroup>
        <Input label={resolvedLabels.name} placeholder={resolvedLabels.namePlaceholder} {...register('name')} error={errors.name?.message} />
        <Input label={resolvedLabels.graduationYear} placeholder={resolvedLabels.graduationYearPlaceholder} {...register('graduationYear')} error={errors.graduationYear?.message} />
        <Select label={resolvedLabels.stream} options={streamSelectOptions} {...register('stream')} error={errors.stream?.message} />
        <Input label={resolvedLabels.currentRole} {...register('currentRole')} error={errors.currentRole?.message} />
        <Input label={resolvedLabels.currentOrg} {...register('currentOrg')} error={errors.currentOrg?.message} />
        <Textarea label={resolvedLabels.quote} rows={3} {...register('quote')} error={errors.quote?.message} />
      </FormFieldGroup>
      ...
      <div aria-hidden="true" className="absolute h-px w-px overflow-hidden opacity-0" style={{ left: '-9999px' }}>
        ...
        <input id="alumni-submit-website" type="text" tabIndex={-1} autoComplete="off" {...register('website')} />
      </div>
      ...
      {submitStatus === 'error' && <p className="text-center text-semantic-error-base font-body text-body-sm">{resolvedLabels.errorMessage}</p>}
    </form>
  );
}
