...
export function buildContactSchema(messages?: { name?: string; email?: string; subject?: string; message?: string }) {
  return z.object({
...
export function ContactForm({ onSubmit, labels, schema, className }: ContactFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)} className={className ?? 'flex flex-col gap-space-6'}>
      <FormFieldGroup>
        <Input label={resolvedLabels.name} {...register('name')} error={errors.name?.message} />
        <Input label={resolvedLabels.email} type="email" {...register('email')} error={errors.email?.message} />
        <Input label={resolvedLabels.subject} {...register('subject')} error={errors.subject?.message} />
        <Textarea label={resolvedLabels.message} rows={5} {...register('message')} error={errors.message?.message} />
      </FormFieldGroup>
