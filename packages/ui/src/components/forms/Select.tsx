...
export const Select = forwardRef<HTMLSelectElement, Props>(({ label, options, helperText, error, disabled = false, required = false, placeholder, value, defaultValue, onChange, onBlur, name, className }, ref) => {
  ...
  return (
    <div className={cn('flex flex-col gap-space-2', className)}>
      <label htmlFor={id} className="font-body text-label text-text-primary uppercase tracking-label">
        {label}
        {required && (
      ...
      <div className="relative">
        <select ref={ref} id={id} name={name} value={value} defaultValue={defaultValue} disabled={disabled} required={required} onChange={onChange} onBlur={onBlur} aria-invalid={hasError} aria-describedby={describedBy} className={cn(selectBase, hasError ? 'border-semantic-error-base focus-visible:border-semantic-error-base focus-visible:outline-semantic-error-base' : 'border-border-default hover:border-border-default focus-visible:border-gold-base')}>
          {placeholder && (
        ...
        <div className="pointer-events-none absolute inset-y-0 right-space-4 flex items-center">
          <svg className="w-4 h-4 text-text-muted" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
