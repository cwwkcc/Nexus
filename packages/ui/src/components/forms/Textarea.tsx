...
export const Textarea = forwardRef<HTMLTextAreaElement, Props>(({ label, helperText, error, disabled = false, required = false, placeholder, rows = 4, value, defaultValue, onChange, onBlur, name, className }, ref) => {
  ...
  return (
    <div className={cn('flex flex-col gap-space-2', className)}>
      ...
      <textarea ref={ref} id={id} name={name} rows={rows} value={value} defaultValue={defaultValue} placeholder={placeholder} disabled={disabled} required={required} onChange={onChange} onBlur={onBlur} aria-invalid={hasError} aria-describedby={describedBy} className={cn(textareaBase, hasError ? 'border-semantic-error-base focus-visible:border-semantic-error-base focus-visible:outline-semantic-error-base' : 'border-border-default hover:border-border-default focus-visible:border-gold-base')} />
      {hasError && (
