...
export function RequirementsChecklist({ title, items, onComplete, className, printLabel = 'Print Checklist', requiredLabel = '*Required' }: RequirementsChecklistProps) {
  ...
  const allRequiredComplete = items.filter((item) => item.required).every((item) => completed.has(item.id));

  return (
    <div className={cn('bg-surface-elevated border border-border-light rounded-lg p-space-6', className)}>
      <div className="mb-space-4 pb-space-4 border-b border-border-light">
        <h3 className="font-display text-h3 mb-space-2">{title}</h3>
        <p className="font-body text-body-sm text-text-muted">{allRequiredComplete ? '✓ All required items completed' : `${items.filter((i) => i.required).filter((i) => completed.has(i.id)).length}/${items.filter((i) => i.required).length} required items completed`}</p>
      </div>
