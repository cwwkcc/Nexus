...
export const Slider = forwardRef<HTMLInputElement, SliderProps>(({ label, name, min = 0, max = 100, step = 1, value, defaultValue = 50, onChange, showValue = true, showMarks = false, disabled = false, className }, ref) => {
  ...
  return (
    <div className={cn('w-full', className)}>
      ...
      */}
      <input ref={ref} type="range" name={name} min={min} max={max} step={step} value={currentValue} disabled={disabled} onChange={(e: ChangeEvent<HTMLInputElement>) => handleChange(Number(e.target.value))} className="sr-only" tabIndex={-1} aria-hidden="true" />
      {(label || showValue) && (
