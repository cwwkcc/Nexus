...
export function ErrorState({ variant = 'inline', message = 'Something went wrong. Please try again.', onRetry, retryLabel = 'Try Again' }: ErrorStateProps) {
  ...
  return (
    <div role="alert" className={cn('flex flex-col', isSection ? 'items-center text-center py-space-16 px-space-6' : 'items-start text-left py-space-5 px-space-6', 'bg-semantic-error-surface border border-semantic-error-base', 'gap-space-3')}>
      {isSection && (
        <div aria-hidden="true" className={cn('w-size-10 h-size-10 rounded-full', 'border border-semantic-error-base', 'flex items-center justify-center', 'text-semantic-error-base text-lg', 'opacity-70')}>
          !
        </div>
      )}
