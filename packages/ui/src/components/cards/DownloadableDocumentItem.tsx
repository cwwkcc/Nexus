  ...
  return (
    <div className={cn('flex items-start gap-4 p-4 bg-surface-default border border-border-light rounded-md', className)}>
      ...
      <div className="flex-1 min-w-0">
        ...
        {description && <p className="font-body text-sm text-text-muted mt-1">{description}</p>}
        <div className="flex items-center gap-3 mt-2">
          <span className="font-body text-caption uppercase text-text-muted">{fileType.toUpperCase()}</span>
          <span className="font-body text-caption text-text-muted">{fileSize}</span>
        </div>
      </div>
