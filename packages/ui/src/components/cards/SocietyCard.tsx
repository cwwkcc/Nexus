    ...
    <Link href={href} className="group block no-underline h-full">
      <div className={cn('h-full flex flex-col bg-surface-elevated border border-border-light overflow-hidden', 'shadow-elevation-1 transition-all duration-gentle', 'group-hover:shadow-elevation-3 group-hover:-translate-y-0.5')}>
        <div className="relative aspect-[4/3] bg-green-base overflow-hidden flex-shrink-0">
          {imageSrc ? (
            <Image src={imageSrc} alt={imageAlt ?? name} fill className="object-cover transition-transform duration-gentle group-hover:scale-[1.05]" />
          ) : (
      ...
      <div className={cn('grid grid-cols-1 md:grid-cols-2 bg-surface-elevated border-2 border-gold-base overflow-hidden', 'shadow-[0_4px_20px_rgba(201,151,58,0.12)] transition-shadow duration-gentle', 'group-hover:shadow-[0_8px_40px_rgba(201,151,58,0.2)]')}>
        <div className="relative min-h-[280px] bg-green-base overflow-hidden">
          {imageSrc ? (
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-green-base to-green-hover flex items-center justify-center">
              <span className="font-display text-6xl italic text-gold-base/30">{name.charAt(0)}</span>
            </div>
          )}
...
export function SocietyCard({ variant = 'hub-grid', ...props }: SocietyCardProps) {
