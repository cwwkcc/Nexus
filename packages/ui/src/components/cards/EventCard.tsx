...
function DateBlock({ date, isPast }: { date: string; isPast?: boolean }) {
  ...
  return (
    <div className={cn('flex flex-col items-center justify-center min-w-[56px] px-space-3 py-space-2.5', isPast ? 'bg-surface-deep' : 'bg-green-base')}>
      <span className={cn('font-display text-[1.75rem] font-medium leading-none', isPast ? 'text-text-muted' : 'text-gold-base')}>{day}</span>
      {month && <span className={cn('font-body text-[0.55rem] uppercase tracking-[0.15em] mt-space-0.5', isPast ? 'text-text-muted' : 'text-text-inverse/60')}>{month}</span>}
    </div>
  );
}
...
function EventCardStandard({ title, description, date, time, venue, category, status = 'upcoming', href, imageSrc, imageAlt, relativeTime, statusLabels }: EventCardProps) {
  ...
  return (
    <Link href={href} className="group block no-underline">
      <div className={cn('bg-surface-elevated border border-border-light overflow-hidden', 'shadow-elevation-1 transition-all duration-gentle', 'group-hover:shadow-elevation-3 group-hover:-translate-y-0.5', isPast && 'opacity-70')}>
        {imageSrc && (
          <div className="relative aspect-[16/7] overflow-hidden bg-green-base">
            <Image src={imageSrc} alt={imageAlt ?? title} fill className={cn('object-cover transition-transform duration-gentle group-hover:scale-[1.03]', isPast && 'grayscale-[0.4] saturate-[0.7]')} />
          </div>
        )}
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 bg-surface-elevated border border-border-light overflow-hidden">
      <div className="relative min-h-[360px] bg-green-base">
        {imageSrc ? (
          <Image src={imageSrc} alt={imageAlt ?? title} fill className={cn('object-cover', isPast && 'grayscale-[0.4] saturate-[0.7]')} />
        ) : (
          <div className="absolute inset-0 bg-gradient-to-br from-green-base to-green-hover flex items-center justify-center">
            <span className="font-display text-6xl text-gold-base/40">{date.split('-')[2] ?? date}</span>
          </div>
        )}
      ...
      <div className="p-space-10 md:p-space-11 flex flex-col justify-center gap-space-5">
        ...
        <div className="flex items-start gap-space-4">
          <DateBlock date={date} isPast={isPast} />
          <div>
            {time && <p className="font-body text-caption uppercase tracking-caption text-text-muted mb-space-1">{time}</p>}
            {relativeTime && <p className="font-body text-body-sm text-gold-base font-medium">{relativeTime}</p>}
          </div>
        </div>
        <div>
