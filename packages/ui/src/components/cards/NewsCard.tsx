    ...
    <Link href={href} className="group block no-underline">
      <div className={cn('bg-surface-elevated border border-border-light overflow-hidden', 'shadow-elevation-1 transition-all duration-gentle', 'group-hover:shadow-elevation-3 group-hover:-translate-y-0.5')}>
        <div className="relative aspect-[16/7] overflow-hidden bg-green-base">
          {imageSrc ? <Image src={imageSrc} alt={imageAlt ?? title} fill className="object-cover transition-transform duration-gentle group-hover:scale-[1.03]" /> : <div className="absolute inset-0 bg-gradient-to-br from-green-base to-green-hover" />}
    ...
    <Link href={href} className="group block no-underline h-full">
      <div className={cn('flex flex-col h-full bg-surface-elevated border border-border-light overflow-hidden', 'shadow-elevation-1 transition-all duration-gentle', 'group-hover:shadow-elevation-3 group-hover:-translate-y-0.5')}>
        <div className="relative aspect-[16/9] overflow-hidden bg-green-base flex-shrink-0">
          {imageSrc ? <Image src={imageSrc} alt={imageAlt ?? title} fill className="object-cover transition-transform duration-gentle group-hover:scale-[1.04]" /> : <div className="absolute inset-0 bg-gradient-to-br from-green-base to-green-hover" />}
...
// Compact variant
function NewsCardCompact({ title, category, date, href, imageSrc, imageAlt }: NewsCardProps) {
  return (
    <Link href={href} className="group block no-underline">
      <div className="flex gap-space-4 py-space-3.5 border-b border-border-light transition-opacity duration-fast group-hover:opacity-80">
        <div className="relative w-[72px] h-14 flex-shrink-0 overflow-hidden bg-green-base">{imageSrc ? <Image src={imageSrc} alt={imageAlt ?? title} fill className="object-cover" /> : <div className="absolute inset-0 bg-gradient-to-br from-green-base to-green-hover" />}</div>
        <div className="flex-1 min-w-0">
          ...
          <h4 className={cn('font-body text-body-sm font-medium text-text-primary line-clamp-2', 'transition-colors duration-fast group-hover:text-gold-active')}>{title}</h4>
        </div>
      </div>
    </Link>
  );
