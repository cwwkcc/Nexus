...
function TrendIndicator({ direction = 'neutral', value, label }: { direction?: TrendDirectionType; value?: string; label?: string }) {
  const config = {
...
export function StatCard({ variant = 'single', target, value, suffix = '', label, trend, trendValue, trendLabel, className }: StatCardProps & { value?: number; trendValue?: string; trendLabel?: string }) {
  ...
  return (
    <div ref={ref as React.RefObject<HTMLDivElement>} className={cn('p-space-7 bg-surface-elevated border border-border-light', 'shadow-elevation-1', className)}>
      ...
      <div className={cn('flex items-baseline gap-space-1', variant === 'with-trend' && trendObj && 'mb-space-3')}>
        <span className="font-display text-[clamp(2rem,4vw,2.8rem)] font-medium text-gold-base leading-none">{numericTarget}</span>
        {suffix && <span className="font-display text-[clamp(1rem,2vw,1.4rem)] font-medium text-gold-base/70 leading-none">{suffix}</span>}
      </div>
      {variant === 'with-trend' && trendObj && <TrendIndicator direction={trendObj.direction} value={trendObj.value} label={trendObj.label} />}
    </div>
  );
}
