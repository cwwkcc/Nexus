  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-12 bg-surface-elevated border border-border-light overflow-hidden">
      <div className="md:col-span-4 relative min-h-[420px] bg-green-base">
        {imageSrc ? (
          <Image src={imageSrc} alt={imageAlt ?? name} fill className="object-cover object-top" />
        ) : (
          <div className="absolute inset-0 bg-gradient-to-br from-green-base to-green-hover flex items-center justify-center">
            <span className="font-display text-7xl italic text-gold-base/25">{name.charAt(0)}</span>
          </div>
        )}
      ...
      <div className="md:col-span-8 p-space-11 flex flex-col justify-center gap-space-5">
        <div>
          <p className="font-body text-caption uppercase tracking-[0.22em] text-gold-base mb-space-2.5">{title}</p>
          <h2 className="font-display text-[clamp(1.8rem,3.5vw,2.8rem)] font-medium text-text-primary mb-space-1.5">{name}</h2>
          {tenure && <p className="font-body text-caption uppercase tracking-caption text-text-muted">{tenure}</p>}
        </div>
        <div className="w-9 h-0.5 bg-gold-base" />
        {quote && (
          <blockquote className="m-0 pl-space-5 border-l-[3px] border-gold-base">
            <p className="font-display text-[clamp(1.1rem,2vw,1.5rem)] italic text-text-primary">&ldquo;{quote}&rdquo;</p>
          </blockquote>
        )}
        {href && (
...
// Grid variant
function StaffCardGrid({ name, title, tenure, portfolio, imageSrc, imageAlt }: StaffCardProps) {
  return (
    <div className="bg-surface-elevated border border-border-light overflow-hidden flex flex-col">
      <div className="relative aspect-[3/4] bg-green-base overflow-hidden">
        {imageSrc ? (
          ...
          <div className="absolute inset-0 bg-gradient-to-br from-green-base to-green-hover flex items-center justify-center">
            <span className="font-display text-4xl italic text-gold-base/30">{name.charAt(0)}</span>
          </div>
        )}
      ...
      <div className="p-space-4 pb-space-5">
        <h4 className="font-display text-h3 font-medium text-text-primary mb-space-1">{name}</h4>
        <p className="font-body text-caption uppercase tracking-caption text-gold-base mb-space-1">{title}</p>
        {portfolio && <p className="font-body text-body-sm text-text-muted">{portfolio}</p>}
        {tenure && <p className="font-body text-caption uppercase tracking-caption text-text-muted mt-space-1.5 opacity-70">{tenure}</p>}
      </div>
    </div>
  );
}
      ...
      <div className="relative w-[52px] h-[52px] rounded-full overflow-hidden bg-green-base border-2 border-border-light flex-shrink-0">
        {imageSrc ? (
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="font-display text-2xl italic text-gold-base/50">{name.charAt(0)}</span>
          </div>
        )}
      </div>
      <div>
        <p className="font-body font-medium text-text-primary mb-space-0.5">{name}</p>
        <p className="font-body text-caption uppercase tracking-caption text-gold-base">{title}</p>
      </div>
    </div>
  );
}
