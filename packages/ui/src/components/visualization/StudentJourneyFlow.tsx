    ...
    <div className={cn('relative overflow-x-auto py-space-8', className)}>
      <svg viewBox="0 0 1000 400" className="w-full min-w-[800px] h-auto" style={{ background: 'var(--surface-base)' }}>
        ...
        {edges.map((edge, idx) => {
          ...
          const isHighlightedEdge = hoveredNode === edge.from || hoveredNode === edge.to || selectedNode === edge.from || selectedNode === edge.to;
          return (
            <g key={idx}>
              <line x1={fromNode.position.x} y1={fromNode.position.y} x2={toNode.position.x} y2={toNode.position.y} stroke={isHighlightedEdge ? 'var(--color-gold-base)' : 'var(--border-light)'} strokeWidth={isHighlightedEdge ? 2 : 1.5} strokeDasharray={isHighlightedEdge ? 'none' : '4'} markerEnd="url(#arrowhead)" />
              {edge.label && (
                <text x={(fromNode.position.x + toNode.position.x) / 2} y={(fromNode.position.y + toNode.position.y) / 2 - 5} textAnchor="middle" className="font-body text-caption fill-text-muted">
                  {edge.label}
                </text>
              )}
            </g>
          ...
          return (
            <g key={node.id} transform={`translate(${node.position.x}, ${node.position.y})`} onMouseEnter={() => setHoveredNode(node.id)} onMouseLeave={() => setHoveredNode(null)} onClick={() => setSelectedNode(isSelected ? null : node.id)} style={{ cursor: 'pointer' }}>
              ...
              {node.streams && node.streams.length > 0 && (
                <text x={0} y={12} textAnchor="middle" className="font-body text-caption fill-text-muted">
                  {node.streams.join(', ')}
                </text>
              )}
              {node.description && isSelected && (
                <>
                  <rect x={-70} y={40} width={140} height={40} rx={4} fill="var(--surface-inverse)" opacity={0.9} />
                  <text x={0} y={62} textAnchor="middle" className="font-body text-caption fill-text-inverse">
                    {node.description}
                  </text>
                </>
              )}
