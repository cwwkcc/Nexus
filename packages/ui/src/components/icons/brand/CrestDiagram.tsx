...
type HotspotPosition = 'top-right' | 'bottom-right' | 'top-left' | 'bottom-left';
...
const DEFAULT_SYMBOLS: CrestSymbol[] = [
  {
    ...
    meaning: 'The oil lamp — Pahana — represents the light of education driving away the darkness of ignorance.',
    position: 'top-right',
  },
  {
    id: 'lotus',
...
export function CrestDiagram({ symbols = DEFAULT_SYMBOLS, variant = 'ambient', className, debug }: CrestDiagramProps) {
  ...
  const calculateLines = useCallback(() => {
    ...
    symbols.forEach((symbol) => {
      const dot = containerRef.current?.querySelector(`[data-dot="${symbol.id}"]`);
      const label = containerRef.current?.querySelector(`[data-label="${symbol.id}"]`);
      if (!dot || !label) return;
      ...
      newLines[symbol.id] = `${labelX},${labelY} ${midX},${labelY} ${midX},${dotY} ${dotX},${dotY}`;
    });
    setLines(newLines);
  }, [symbols]);
    ...
    <div ref={containerRef} className={cn('relative select-none w-size-screen-w-80 aspect-[190/100]', 'my-space-2 bg-surface-active rounded-md overflow-hidden', className)}>
      ...
      <svg className="absolute inset-0 w-full h-full pointer-events-none z-base" viewBox={`0 0 ${containerSize.width} ${containerSize.height}`} aria-hidden>
        ...
        {symbols.map((symbol) => {
          ...
          const shouldAnimate = variant === 'ambient' || (isHovered && !isActive);

          return (
            <motion.polyline
              key={symbol.id}
              points={points}
              className={cn('fill-none transition-colors duration-300 ease-out', isActive ? 'stroke-gold-base' : isHovered && variant !== 'ambient' ? 'stroke-gold-base/60' : 'stroke-border-strong/70')}
              strokeWidth={isActive ? 2.5 : 2}
              strokeDasharray={isActive && variant !== 'ambient' ? '0' : '4 4'}
              strokeLinejoin="round"
              animate={shouldAnimate ? { strokeDashoffset: [0, -16] } : { strokeDashoffset: 0 }}
              transition={{
                repeat: Infinity,
