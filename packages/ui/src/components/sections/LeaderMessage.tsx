...
export function LeaderMessage({ name, title, tenure, portraitSrc, portraitAlt, message, quote, fullMessageHref, fullMessageLabel = 'Read Full Message →', className }: LeaderMessageProps) {
  return (
    <div className={clsx('grid md:grid-cols-2 gap-12 items-center', className)}>
      ...
      {/* Message */}
      <div>
        <p className="font-body text-eyebrow uppercase tracking-wider text-gold-base mb-2">{title}</p>
        <h2 className="font-display text-h2">{name}</h2>
        <p className="font-body text-caption text-text-muted uppercase tracking-wider mb-6">{tenure}</p>

        <div className="w-size-12 h-size-0p5 bg-gold-base mb-space-6" />

        {quote && <QuoteBlock variant="pull-quote" quote={quote} className="mb-space-6" />}
