...
export function CategorizedPhotoStrip({ items, className, allLabel = 'All', categoryLabels = DEFAULT_CATEGORY_LABELS, scrollLeftLabel = 'Scroll left', scrollRightLabel = 'Scroll right' }: CategorizedPhotoStripProps) {
  ...
  return (
    <div className={cn('relative', className)}>
      ...
      <div className="flex justify-center gap-2 mb-8 flex-wrap">
        <button onClick={() => setActiveCategory('all')} className={cn('px-4 py-2 font-body text-label uppercase tracking-wide rounded-full transition-all', activeCategory === 'all' ? 'bg-green-base text-text-inverse' : 'bg-surface-default text-text-muted hover:bg-surface-deep')}>
          {allLabel}
        </button>
        {Object.entries(categoryLabels).map(([key, label]) => (
          <button key={key} onClick={() => setActiveCategory(key)} className={cn('px-4 py-2 font-body text-label uppercase tracking-wide rounded-full transition-all', activeCategory === key ? 'bg-green-base text-text-inverse' : 'bg-surface-default text-text-muted hover:bg-surface-deep')}>
            {label}
          </button>
        ))}
      </div>
          ...
          {filteredItems.map((item) => (
            <div key={item.id} className="flex-shrink-0 w-72 md:w-80 group cursor-pointer">
              <div className="relative overflow-hidden rounded-lg">
                <ImageFrame src={item.imageSrc} alt={item.imageAlt} aspectRatio="news" variant="standard" className="transition-transform duration-500 group-hover:scale-105" />
