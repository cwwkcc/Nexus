...
export function Toast({ variant, message, visible, onDismiss, duration = 5000, dismissLabel = 'Dismiss notification' }: ToastProps) {
  ...
  return (
    <div role="status" aria-live="polite" aria-atomic="true" className={cn('fixed bottom-space-6 right-space-6 z-toast', 'flex items-center gap-space-3 p-space-3.5', 'min-w-[280px] max-w-[400px]', s.bg, s.border, 'border shadow-elevation-2', 'transition-all duration-gentle ease-out', visible ? 'translate-y-0 opacity-100 pointer-events-auto' : 'translate-y-6 opacity-0 pointer-events-none')}>
      <span aria-hidden="true" className={cn('w-5 h-5 rounded-full border-[1.5px] border-current', 'flex items-center justify-center', s.text, 'text-xs font-bold flex-shrink-0')}>
        {s.icon}
      </span>
