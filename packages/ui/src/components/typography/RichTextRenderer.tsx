...
function extractPlainText(node: TiptapNode | undefined): string {
  ...
  return node.content.map((child) => (child.type === 'text' ? (child.text ?? '') : extractPlainText(child))).join('');
}
...
function renderNode(node: TiptapNode, key: number): ReactNode {
  ...
  switch (node.type) {
    case 'paragraph':
      return (
        <p key={key} className="font-body text-body text-text-primary mb-space-4 leading-relaxed">
          {children()}
        </p>
      );
    ...
    case 'heading': {
      ...
      if (level <= 2) {
        return (
          <h2 key={key} className="font-display text-h2 mt-space-8 mb-space-4 text-text-primary">
            {children()}
          </h2>
        );
      }
      if (level === 3) {
        return (
          <h3 key={key} className="font-display text-h3 mt-space-6 mb-space-3 text-text-primary">
            {children()}
          </h3>
        );
      }
      return (
        <h4 key={key} className="font-display text-h3 mt-space-4 mb-space-2 text-text-primary">
    ...
    case 'bulletList':
      return (
        <ul key={key} className="flex flex-col gap-space-1 list-disc pl-space-6 mb-space-4">
          {children()}
        </ul>
      );
    ...
    case 'orderedList':
      return (
        <ol key={key} className="flex flex-col gap-space-1 list-decimal pl-space-6 mb-space-4">
          {children()}
        </ol>
      );
    ...
    case 'blockquote':
      return <QuoteBlock key={key} variant="pull-quote" quote={extractPlainText(node)} />;

    case 'image':
      return (
        <figure key={key} className="my-space-6">
          <ImageFrame src={(node.attrs?.src as string) ?? ''} alt={(node.attrs?.alt as string) ?? ''} aspectRatio="hero" variant="standard" />
          {node.attrs?.title ? <figcaption className="text-center font-body text-caption text-text-muted mt-space-2">{node.attrs.title as string}</figcaption> : null}
        </figure>
      );
...
export function RichTextRenderer({ value, className }: RichTextRendererProps) {
  ...
  return <div className={cn('max-w-none', className)}>{doc.content.map((node, i) => renderNode(node, i))}</div>;
}
