...
export const InlineLink = ({ href, children, external = false, prefetch = true, className, ref, ...rest }: InlineLinkProps) => {
  if (external) {
    return (
      <a href={href} target="_blank" rel="noopener noreferrer" className={cn(inlineLinkStyles, className)} {...rest}>
        {children}
  ...
  return (
    <Link href={href} prefetch={prefetch} className={cn(inlineLinkStyles, className)} {...rest}>
      {children}
    </Link>
  );
};
