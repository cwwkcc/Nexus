...
export function ButtonLink({ href, variant = 'primary', size = 'md', disabled = false, fullWidth = false, leftIcon, rightIcon, children, className, onClick, ref, ...rest }: ButtonLinkProps) {
  ...
  return (
    <a ref={ref} href={disabled ? undefined : href} aria-disabled={disabled || undefined} tabIndex={disabled ? -1 : undefined} data-variant={variant} data-size={size} data-disabled={disabled ? '' : undefined} data-full-width={fullWidth ? '' : undefined} data-icon-only={size.startsWith('icon') ? '' : undefined} onClick={handleClick} className={cn(base, variants[variant], sizes[size], fullWidth && 'w-full', disabled && 'opacity-40 cursor-not-allowed pointer-events-none', !disabled && 'cursor-pointer', className)} {...rest}>
