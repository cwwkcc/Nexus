...
const variants: Record<ButtonVariant, string> = {
  ...
  secondary: 'bg-surface-base text-text-primary border-green-base hover:border-gold-hover hover:text-gold-hover active:border-gold-active active:text-gold-active',
  ghost: 'bg-transparent text-gold-base border-transparent hover:border-gold-hover hover:text-gold-hover active:border-gold-active active:text-gold-active',
  outline: 'bg-transparent text-gold-base border-gold-base hover:border-gold-hover hover:text-gold-hover active:border-gold-active active:text-gold-active',
  destructive: 'bg-semantic-error-base text-text-inverse border-transparent hover:bg-semantic-error-base/80 active:bg-semantic-error-base/60',
...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(({ variant = 'primary', size = 'md', type = 'button', loading = false, loadingText, disabled = false, fullWidth = false, leftIcon, rightIcon, children, className, onClick, ...rest }, ref) => {
  ...
  return (
    <button ref={ref} type={type} disabled={isDisabled} aria-busy={loading} data-variant={variant} data-size={size} data-loading={loading ? '' : undefined} data-disabled={isDisabled ? '' : undefined} data-full-width={fullWidth ? '' : undefined} data-icon-only={size.startsWith('icon') ? '' : undefined} onClick={handleClick} className={cn(base, variants[variant], sizes[size], fullWidth && 'w-full', isDisabled && 'opacity-40 cursor-not-allowed', !isDisabled && 'cursor-pointer', className)} {...rest}>
      {loading && (
        <span className="absolute inset-0 flex items-center justify-center gap-space-2 pointer-events-none">
          ...
          {loadingText && <span className="text-label font-body">{loadingText}</span>}
        </span>
      )}
      ...
      <span className={cn('inline-flex items-center gap-space-2', loading && 'opacity-0')}>
        {leftIcon && <span className="shrink-0 [&>svg]:size-[1em]">{leftIcon}</span>}
        {children}
        {rightIcon && <span className="shrink-0 [&>svg]:size-[1em]">{rightIcon}</span>}
      </span>
    </button>
  );
});
