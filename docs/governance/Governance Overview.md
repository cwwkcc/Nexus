# Nexus — Governance Overview

**Policies and procedures for content management**

---

## Purpose

This governance framework ensures that Nexus remains a living, accurate, and professional digital institution. It defines:

- **Who** is responsible for what content
- **How** content is created, reviewed, and published
- **When** content should be updated or archived
- **What** standards content must meet

---

## Core Principles

### 1. Ownership

Every piece of content has a named owner responsible for its accuracy and timeliness.

### 2. Quality

All content meets established standards for writing, accessibility, and visual identity.

### 3. Timeliness

Content is reviewed regularly and updated promptly when information changes.

### 4. Accountability

All changes are tracked in the audit log. Every editor is accountable for their actions.

### 5. Accessibility

Content is accessible to all users, regardless of ability or device.

---

## Governance Documents

| Document | Purpose |
|----------|---------|
| **[Content-Governance.md](./Content-Governance.md)** | Content ownership and update schedules |
| **[Roles-and-Responsibilities.md](./Roles-and-Responsibilities.md)** | Staff and editor roles |
| **[Content-Approval-Workflow.md](./Content-Approval-Workflow.md)** | Review and publishing process |
| **[Content-Guidelines.md](./Content-Guidelines.md)** | Writing, image, and accessibility standards |
| **[Accessibility-Policy.md](./Accessibility-Policy.md)** | WCAG 2.1 AA compliance |
| **[Editorial-Style-Guide.md](./Editorial-Style-Guide.md)** | Tone, voice, and style |
| **[Social-Media-Governance.md](./Social-Media-Governance.md)** | Social media policies |
| **[Data-Privacy-Policy.md](./Data-Privacy-Policy.md)** | Student and staff data protection |
| **[Content-Review-Schedule.md](./Content-Review-Schedule.md)** | Regular review cycles |

---

## Sign-off

This governance framework is approved by:

| Role | Name | Signature | Date |
|------|------|-----------|------|
| Principal | | | |
| Staff Advisor | Mrs. Tharindrie Perera | | |
| KITS Lead | | | |
| Editorial Team Lead | | | |

---

**C.W.W. Kannangara Central College, Est. 1873. "Wisdom is All Wealth."**
