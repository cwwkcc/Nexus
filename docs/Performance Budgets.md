...
Nexus must be fast on all devices, especially on mobile networks in Sri Lanka. This document defines page‑specific performance budgets that will be enforced via **Lighthouse CI** and **bundler analysis**.

All budgets refer to **initial page load** (first view, uncached). Subsequent views may have cached assets.
...
|------|-----------|--------------|--------------|-------|
| Home | ≤ 200KB | ≤ 1.5MB | ≤ 2MB | Hero image optimised; lazy load below‑fold images |
| About | ≤ 150KB | ≤ 1MB | ≤ 1.5MB | Timeline images lazy loaded |
| Administration | ≤ 150KB | ≤ 800KB | ≤ 1.2MB | Staff portraits lazy loaded |
| Academics | ≤ 150KB | ≤ 800KB | ≤ 1.2MB | Stream comparison table interactive |
...
| Contact | ≤ 100KB | ≤ 300KB | ≤ 500KB | No hero image, static map |
...
- No external libraries (e.g., Google Tag Manager, Facebook Pixel) unless approved.
- All third‑party scripts must be audited for performance impact.
...
- Use dynamic imports for heavy components:
  - `CrestDiagram` (loaded only on about page)
  - `PanoramicFacilityViewer` (loaded only on facilities page)
  - `AudioPlayer` (loaded only on about page)
  - `Lightbox` (loaded only on gallery page)
...
- **Sentry** – performance monitoring (transaction traces, slow page loads).
- **Vercel Analytics** – Core Web Vitals real‑user monitoring (if enabled).
- **Umami** – page load times (via `navigation` timing API, if custom event added).
...
5. Re‑run Lighthouse CI to verify.

Budgets are enforced at PR time via GitHub Actions.

---

## Related Documents

- [Foundations](./Design%20System/Foundations.md) – performance rules
- [Page Specifications](./Design%20System/Page%20Specifications.md) – asset expectations per section
