...
> **⚠️ Open discrepancy to resolve before this figure is final:** the "Hetzner Storage Box" line above assumes backups are stored there, but the backup automation actually built (`infra/scripts/backup.sh`) uploads encrypted backups to **AWS S3** (`s3://nexus-backups-bucket`), not a Hetzner Storage Box. AWS S3 has its own real, ongoing cost that isn't itemized anywhere in this table. Before this budget is treated as final, KITS needs to either (a) point the backup script at the already-budgeted Hetzner Storage Box instead, or (b) replace that line item with an actual AWS S3 cost estimate. Either way, the number above should be double-checked, not assumed accurate as written.
