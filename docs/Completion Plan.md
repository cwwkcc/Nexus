...
- [x] Model + migration (check contracts: `domains/extracurriculars/`)
- [x] Router
- [x] Admin module
- [x] Public page

> **Data shape decision:** `ExtracurricularActivity`, locale-scoped per row, no slug, no individual page — Announcement's shape, not Society's. `description`/`studentQuote` are prose needing translation (the usual per-locale signal), but F-161's own field list (a single listing page, cards grouped by category) never asks for an individually routable detail page the way F-148/F-152/F-146 do for Society/Gallery/EventDetail. `coachStaffId` is a real FK to Staff (F-179: "coach/advisor via staff selection"), nullable/`SetNull` like Society's `advisorStaffId`. `isActive` is F-179's one real structural departure from Society/Staff/GalleryAlbum's "no draft/published state" precedent — F-179's own words, "Unlike Academic Programs, activities genuinely get added and retired over the years," describe a permanent retirement flag, not a draft-review workflow, so it reuses Announcement's plain-Boolean `isActive` rather than `ContentStatus`. Achievements are a second, related model (`ExtracurricularAchievement`, cascade-deleted with its activity) reconciled as a single array on the activity's own create/update input — the same photos-on-an-album pattern Gallery already established — not separate per-achievement mutations, and not the same entity as the school-wide `Achievement` model Task 7.19 will add (see `editorial/achievements/achievement.ts`'s own header comment for that distinction). Full reasoning in `schema.prisma`'s own `ExtracurricularActivity`/`ExtracurricularAchievement` doc comments.
>
> **Verification note (2026-08-11):** Same sandbox constraints as every prior pass — `prisma generate` still 403s against `binaries.prisma.sh`. Extended the hand-written stub client with precisely-modeled `ExtracurricularActivityRow`/`ExtracurricularAchievementRow` (plus, while in there, three unrelated typing gaps the stub hadn't needed to model yet: `EventDetail.calendarEntry`/`GalleryAlbum.photos`/`ContentEntryVersion.contentEntry` relation-include shapes, `Staff.aggregate` for its per-role `order` append, and a true tuple-preserving `$transaction` overload for `setup-totp/actions.ts`'s heterogeneous `[user.update, backupCode.createMany]` array — all three pre-existing call sites the stub simply hadn't been exercised against until this pass's full-graph `tsc -b` run reached them). **Zero** type errors across `@nexus/contracts`, `@nexus/database`, `@nexus/api` (all nine modules), `apps/admin`, and `apps/web` — the same four pre-existing `apps/web` errors as every pass since Events (confirmed unchanged via `git diff` on those exact files — none touched by this pass — plus `@tiptap/react` confirmed genuinely absent from `apps/web/package.json`, not a stub gap), none introduced by this one. `packages/api/src/modules/extracurriculars/__tests__/router.test.ts` executed (`tsx --test`), 9/9 — DB-failure degradation on `list`/`adminList`, `adminGetById` genuinely throwing (not degrading) on failure, RBAC gating on `create`/`delete`, the `update`-is-not-admin-only distinction (an Editor toggling `isActive` to retire a team), and Zod-layer validation of achievement dates/categories/id-present-means-update. `apps/admin/src/lib/extracurriculars.test.ts` (4/4, category and achievement-level label fallbacks) also executed and passes. Full `packages/api` suite re-run end to end: **71/71**, confirming nothing regressed. `apps/admin`'s full `src/lib/*.test.ts` suite: **35/35**. Scoped `eslint` runs (each package/app through its own `eslint.config.mjs`, not the root one — running from root produces a wall of `tailwindcss/no-custom-classname` false positives because the root config doesn't carry `apps/admin`'s/`apps/web`'s own Tailwind config path): zero errors anywhere in this pass's files; the only warnings are ones already present at identical severity in the precedent files being followed (`console.error` in every `service.ts`'s degrade-to-`[]` catch block, `initial!.id` in every edit-mode form's submit handler, an "unused eslint-disable" on the `@next/next/no-img-element` comment already present verbatim in `SocietyForm.tsx`/`AlbumForm.tsx`).
>
> **Also decided, not just implemented:**
>
> - **No `order` field, unlike Staff/GalleryAlbum.** F-179's spec doesn't ask for drag-and-drop reordering the way F-165/F-168 explicitly do; both the public and admin lists sort by category then name — deterministic without inventing a field nothing requested. Achievements likewise have no `order` — they're always sorted newest-first by `date`, which is what "recent achievements" on a card means anyway.
> - **The stub admin page comments' "F-166"/"F-147" and "ExtracurricularsJoinCTA section" were wrong/invented, not real requirements.** The real feature IDs are F-161 (public page) and F-179 (admin module) — F-166 is Calendar & Events, F-147 is the Societies Hub. There is no "JoinCTA" component anywhere in `@nexus/ui` and no such section in "Design System/Page Specifications.md" section 09 — it doesn't appear in the built page, flagged in that page's own header comment rather than either fabricated or silently dropped.
> - **`extracurriculars.sports`/`.performingArts`/`.leadership` removed from the page registry**, the same ContentEntry-placeholder cleanup already done for News/Events — a real router now serves every activity, so there's nothing left for an editor to hand-author as raw JSON through the generic block-content system. Only `extracurriculars.hero` (page chrome) remains.
> - **`ExtracurricularCard`'s `variant` prop ('sport', singular) is deliberately a different string than `category` ('sports', plural, the stored/filtered taxonomy value)** — not a bug to "fix," a real mapping (`CATEGORY_TO_CARD_VARIANT` in `apps/web/src/lib/extracurricular-card.ts`) between a filtering/grouping value and a presentational one.
> - **`LifeAtKCCPhotoStrip` (page spec's `CategorizedPhotoStrip`) not wired into the Sports section.** It needs a real "gallery photos tagged by category" data source that doesn't exist — `GalleryAlbum`/`GalleryPhoto` (Task 7.7) share no category taxonomy with Extracurriculars', and building one is meaningfully separate scope F-179's own field list never asked for. Flagged in the public page's own header comment rather than faked with placeholder images.
> - **Coach names resolved via `staff.byId`, called once per distinct coach across the whole activity list, not a new bulk endpoint.** Same public single-lookup procedure `getAdvisor` (Societies) already established; a bulk "staff by ids" procedure is scope this deliberately-unpaginated, few-dozen-rows-at-most module doesn't need yet.
>
> **Not done:** no live Postgres, no browser — same standing limitation as every prior milestone. The Media Library photo picker, the achievement add/remove form interactions, and the public page's category grouping have all been read carefully and typechecked but not clicked through.

**Alumni** (Task 7.18)

- [x] `AlumniProfile` model + migration
- [x] `alumniRouter`
- [x] Admin Alumni module
- [x] Public Alumni Directory page

> **Found already drafted, not verified.** Unlike every other M4 module so far, this one's files (model/migration, `alumniRouter`, admin pages, public directory page) already existed in the repo before this pass started — from an earlier, unlogged session — but the Completion Plan still had all four boxes unchecked, and for good reason: `packages/api/src/modules/alumni/service.ts` did not compile. Real bugs, not stylistic:
>
> - `export type AlumniStatusUpdate = z.infer<typeof AlumniStatusUpdate>;` redeclared an identifier already brought into scope via `import type { ..., AlumniStatusUpdate, ... }`, and took `typeof` of a type-only alias — both illegal. Same for `AlumniBulkStatusUpdate`. Fixed by importing the actual `*Input` schema consts (matching every other module's own convention) and deriving local type aliases from those instead.
> - Every `triggerRevalidation(config, ['alumni'])` call used a signature that has never existed — the real function takes one `{ webAppUrl, secret, scope }` object (`packages/config/src/cache.ts`). Replaced with a `revalidateAlumni()` helper mirroring `announcementsService.revalidateAnnouncements`.
> - Four Prisma error-narrowing checks used bare `err instanceof Prisma.PrismaClientKnownRequestError && err.code === ...` without the `(err as Prisma.PrismaClientKnownRequestError)` cast every other module needs — see M3's own verification note on why plain narrowing doesn't work in this project's TS setup.
> - `errors.ts` threw plain `Error` subclasses instead of real `TRPCError`s via the shared `errors/trpc-errors.ts` helpers, so a missing-profile lookup surfaced as a generic `INTERNAL_SERVER_ERROR` instead of `NOT_FOUND` — `apps/admin/src/app/alumni/actions.ts`'s own `messageFor()` branches on `err.code === 'NOT_FOUND'` specifically and would never have hit that branch. Rebuilt against `notFoundError`/`conflictError`/`internalError`.
>
> **F-180's public submission path was missing entirely.** `alumniRouter` only had admin-gated procedures; `apps/web/src/blocks/alumni/SubmitProfileBlock.tsx` was a two-line comment stub referencing an `alumniRouter.submit()` that didn't exist. Added:
>
> - `AlumniSubmitInput` (`modules/alumni/validators.ts`) — a `.pick()` off the existing admin schema, so a submission is held to the same field validation as an admin-entered one, while structurally excluding `status`/`isFeatureworthy`/`rejectionReason`/`portrait`.
> - `submitProfile()` (`service.ts`) — the first public mutation in this codebase (every other router's mutations are admin-gated). Forces the new row to `status: 'PENDING'`, `isFeatureworthy: false`, `rejectionReason: null`, and no portrait, **server-side, regardless of input** — the moderation queue is the actual publication gate, not client input (`listPublicAlumni` only ever shows `APPROVED` profiles). Deliberately excludes a portrait upload entirely: the Media Library's presigned-upload pipeline (`modules/media/router.ts`) is admin-only by design (F-067), and a second, public-facing upload path is real new scope (abuse surface, storage cost, moderation of uploaded images) beyond this task. An admin can attach a portrait when reviewing the submission. Never throws on a genuine save failure — logs and returns `{ success: false }` rather than leaking a 500 to an anonymous caller.
> - `AlumniSubmitForm` (new component, `packages/ui/src/components/forms/`) — mirrors `ContactForm`/`FeedbackForm`'s exact shape (react-hook-form + zod, no knowledge of any API route). Includes a honeypot field as a first line of defence against simple form-filling bots (fakes the success state and never calls `onSubmit` if tripped); explicitly documented as a modest, client-side-only defence, not a complete one — the real safety boundary is `submitProfile`'s server-side status-forcing above, which holds even if a bot bypasses the form and calls the mutation directly. No IP-based rate limiting exists anywhere in this codebase yet; noted as a real, known gap rather than silently declared solved.
> - `SubmitProfileBlock.tsx` rebuilt for real (was the stub), plus `apps/web/src/app/[locale]/alumni/actions.ts` (a `'use server'` action — apps/web's first — calling `alumni.submit` with a null session) and new i18n strings in `alumni-i18n.ts` (en/si/ta) for the form's labels and success/error messages.
>
> **Other real bugs found and fixed while completing this task:**
>
> - The admin list page's "Search by name, role, or organization…" box sent a `query` URL param that `AlumniListInput` never accepted — the search box did nothing. Added a real `query` field (same `.trim().max(200)` shape as News's own) and wired it into `listAlumni`'s `where` clause alongside `profession`, combined via `AND` (not overwriting each other's `OR`, which a naive second `where.OR = [...]` assignment would have done).
> - `AlumniForm.tsx` imported a non-existent `AlumniStatusEnumData` from `@nexus/contracts` and typed local `stream` state against the narrow `ALStreamEnumData` union when the wire type (`AdminAlumni['stream']`) is plain `string | null` — both real compile errors. Fixed by importing the already-exported `AlumniStatus` type from `apps/admin/src/lib/alumni.ts` and relaxing `stream`'s local state to `string`, matching how every other free-text field in the same form is typed.
> - `actions.ts` called `revalidatePath('/alumni')` twice in `createAlumniProfile`.
> - `apps/admin/src/features/shell/Sidebar.tsx` never listed Alumni at all — the module was unreachable from the admin UI navigation. Added.
> - `packages/contracts/src/system/rbac/matrix.ts` had no `alumni` resource entry. Added, shaped like `events` (not `staff`/`societies`): the PENDING/APPROVED/REJECTED moderation workflow is a real publish gate, so an Editor holds `publish` (can approve/reject a submission) but not `admin` (hard delete stays admin-only, same "nothing to fall back to" reasoning as every other hard-delete in this matrix).
>
> **Bigger-than-expected discovery:** Achievements (Task 7.19) and Archive (Task 7.20) — both still fully unchecked below — turned out to have the _exact same_ bug pattern as Alumni's `service.ts`/`errors.ts` (same import-collision, same wrong `triggerRevalidation` call shape, same missing casts), clearly written in the same earlier, never-verified session. `packages/contracts/src/domains/archive/archive.ts` additionally had genuine duplicate-identifier errors (`ArchiveCategoryData` declared twice, `ARCHIVE_CATEGORY_LABELS` exported twice), and `domains/archive/index.ts` didn't exist at all — Archive's contracts were never wired into the `@nexus/contracts` barrel, so nothing importing `@nexus/contracts` as a whole could ever have compiled. Since `packages/api` and `@nexus/contracts` are each a single TypeScript project, these were blocking a clean build for _every_ module, Alumni included — not something optional to skip. Applied the same minimal, mechanical, behavior-preserving fix as Alumni's own (fixed imports, fixed `triggerRevalidation` calls, fixed Prisma casts, added the missing `AchievementOutput`/`ArchiveOutput` exports and the missing contracts barrel file) purely so both packages compile — **without** touching either module's admin UI, without writing their tests, and without checking any of their own boxes below. Task 7.19/7.20 remain exactly as unstarted as they were; this should not be read as progress on either.
>
> **Verification note (2026-08-21):** Same sandbox constraint as every prior pass — `prisma generate` still 403s against `binaries.prisma.sh`. Rebuilt the hand-written stub client from scratch this pass (rather than extending a checked-in one, since none was present) — all 22 models mirrored against `schema.prisma`'s scalar columns, a generic `Delegate<Row>` class rather than 22 near-identical hand-written classes, a real `namespace Prisma { class PrismaClientKnownRequestError ... }` (declaring the error classes _inside_ the namespace, which gives both the `instanceof`-checkable value and the cast-to-able type for free — an `export import` alias was tried first and rejected two type errors in, before landing on namespace-nested class declarations), and a numeric-only `AggregateResult` (real Prisma's own type is wider, but every `.aggregate()` call in this codebase aggregates a numeric `order` column, and a wider union broke the `+ 1` arithmetic every reorder handler does). **Zero** type errors across `@nexus/contracts`, `@nexus/database`, `@nexus/config`, `@nexus/api`, `@nexus/ui`, `apps/admin`, and `apps/web` (full `tsc -b`/`tsc --noEmit` chain, not just the modules this task touched) — the same six pre-existing `apps/web`/`apps/admin` errors as every recent pass (`AchievementForm.tsx`/`ArchiveForm.tsx` in admin; `achievements/page.tsx`, `news/[slug]/page.tsx`, both admissions blocks, and `server/achievements.ts`/`server/archive.ts` in web — confirmed unchanged via `git diff` on those exact files, none touched by this pass), none introduced by this one.
>
> Full test execution this time used `tsx --test` rather than a manual `tsc` + `node --test` compile, after the latter hit a real but entirely unrelated blocker: `@nexus/tokens`'s own `src/index.ts` has several relative exports (`./semantic`, `./generators/*`, `./themes/*`) without explicit file extensions, which only surfaces as a problem under strict Node ESM resolution (`moduleResolution: nodenext`) — `@nexus/tokens` is designed exclusively for bundler consumption (Next.js/webpack), not for being run directly by plain `node`, and every M4 module's service.ts transitively pulls it in via `triggerRevalidation`'s `@nexus/config` import, which also re-exports a Tailwind preset that needs `nexusTheme` from tokens. `tsx`'s esbuild-based resolution doesn't have this limitation and sidesteps it entirely — confirmed this is genuinely pre-existing and not Alumni-specific by running the _whole_ `@nexus/api` suite, not just the new alumni tests. `packages/api`'s full suite: **81/81** (`NEXT_PUBLIC_SITE_URL`/`NEXT_PUBLIC_ADMIN_URL` set as dummy values for the run — required by `@nexus/env`'s client schema but absent from `.env.example`, another small pre-existing gap noted but not chased further). New `modules/alumni/__tests__/router.test.ts` (10/10): DB-failure degradation on `list`/`adminList`, `adminGetById` genuinely throwing (not degrading) on failure, `submitProfile` never throwing even on DB failure (reports `{ success: false }` instead — pinned down explicitly since it's the one write path in this module that deliberately doesn't follow every other mutation's throw-on-failure convention), RBAC gating on `create`/`delete`, the `setStatus`-is-not-admin-only distinction (an Editor moderating a submission), and Zod-layer checks that `AlumniSubmitInput` actually strips `status`/`isFeatureworthy`/`rejectionReason`/`portrait` from a payload that tries to sneak them in. `apps/admin`'s full `src/lib/*.test.ts` suite: **39/39**, including new `alumni.test.ts` (4/4: `getGraduationYears`'s dedup-and-sort, `ALUMNI_STATUS_LABELS` coverage, `statusOptions`' "all statuses" lead-in).
>
> **Not done:** no live Postgres, no browser — same standing limitation as every prior milestone. The submission form's honeypot behaviour, the admin moderation queue's approve/reject buttons, and the public directory's search/filter UI have all been read carefully and typechecked but not clicked through. Task 7.19 (Achievements) and 7.20 (Digital Archive) remain unstarted beyond the minimal compile-blocking fixes described above — next up, in order.
