...
**F-007 · Environment Variable Documentation** `.env.example` files in the root and in each app document every required environment variable with its purpose and source. A developer who has never seen the project can know exactly what secrets are needed without reading the codebase. This is the documentation half only; the runtime validation and typing of those variables is a separate, undocumented-until-now package — see F-199.
...
**F-011 · Build-Time Automation Framework** Scripts in `packages/tokens/scripts/` automate repetitive generation tasks. This feature covers the infrastructure: the script runner, watch mode, and the convention for how generation scripts are structured and triggered. Two specific outputs live under this umbrella: the token CSS generator (F-019) and the `cn` group generator that creates the typed class name utility. Running these ensures generated output is always in sync with source definitions. **Correction:** these scripts do not live in `packages/config` — they were moved to `packages/tokens` when the token system itself was split out of `packages/config` into its own package (see Group 2 header note below).

**F-199 · Typed, Validated Environment Variables (`packages/env`)** A dedicated package, not previously declared anywhere in this registry, that replaces "hope the `.env` file is right" with a hard failure at boot. Three entry points split by where it's safe to run: `shared.ts` (just `NODE_ENV` — deliberately minimal, with a note in the source about why `APP_NAME`/`APP_URL` aren't included), `server.ts` (`DATABASE_URL`, `NEXTAUTH_*`, `R2_*`, `RESEND_API_KEY`, `ADMIN_API_SECRET`, `REVALIDATE_SECRET`, `ADMIN_EMAIL`/`ADMIN_PASSWORD`, `SENTRY_DSN`, `UMAMI_WEBSITE_ID` — guarded by the `server-only` package so a client bundle fails to build if it's ever imported there), and `client.ts` (`NEXT_PUBLIC_*` only). All three are parsed once at import time via Zod, not behind a `validateEnv()` call someone has to remember to invoke — a missing or malformed variable fails the app at startup with every problem listed at once, rather than failing lazily the first time some code path happens to reach for it. `index.ts` deliberately does not re-export `server.ts`, so a client component can't accidentally pull in a server secret through the top-level import. This is the runtime counterpart to F-007's `.env.example` documentation — F-007 tells a developer what to put in the file; this validates and types what's actually in it. The security fixes made to this package early on (an unset `REVALIDATE_SECRET` silently leaving an endpoint open, `NEXT_PUBLIC_SITE_URL` silently falling back to a hardcoded URL in production) predate this registry entry — the package existed and had already been hardened, it simply had never been written down here.

---

## Group 2 — Design System Tokens (`packages/tokens`)

**Correction to this group's package attribution:** every entry below was originally written as living in `packages/config`. It no longer does. The token system — primitives, semantic mappings, themes, and all five generators — was split out into its own package, `packages/tokens`. `packages/config`'s own README is explicit about this: _"Design tokens have moved to `@nexus/tokens`. Environment variable validation has moved to `@nexus/env`."_ What actually remains in `packages/config` today is the Tailwind preset (F-018, which imports from `@nexus/tokens`), shared Next.js config for both apps, cache utilities, metadata helpers, path constants, and font loading — build/framework tooling, not the tokens themselves. The substance of F-012–F-017 (what the tokens contain) is still accurate; only the package name in each was wrong.

**F-012 · Color Token System** The complete institutional color palette defined as named tokens in `packages/tokens/src/primitives/colors.ts` and `semantic/colors.ts`: forest greens (primary brand), gold accents, semantic colors (success, warning, error, info), and text hierarchy. No hardcoded hex values anywhere in the codebase after this is established.
...
**F-018 · Tailwind nexusPreset** A single Tailwind preset in `packages/config/src/tailwind/preset.ts` maps every token from `@nexus/tokens` into Tailwind's theme. Both `apps/web` and `apps/admin` consume this preset — one source of truth for the entire visual language expressed as utility classes. This is the one piece of the original F-018 description that was accurate as written: the preset genuinely stays in `packages/config`, it's the raw token definitions around it that moved.

**F-019 · Token Generator Pipeline** Five pure-function generators in `packages/tokens/src/generators/` turn the TypeScript token source into everything else that consumes it: `css.ts` produces the `tokens.css` custom-property file imported once in each app's `global.css`; `tailwind.ts` produces the Tailwind theme object F-018's preset maps in; `cn.ts` produces the typed `cn()` utility class groups consumed by `packages/ui`; `json.ts` produces a flat token list for documentation and debugging. A fifth, `figma.ts`, is a deliberate stub — there's no Figma file or designer workflow yet, so the file exists only as an extension point, not a gap to close. Tokens are defined in TypeScript (type-safe, auditable) and consumed in CSS or Tailwind config (zero runtime cost). This is one specific deliverable under the Build-Time Automation Framework (F-011); the automation infrastructure itself belongs to F-011, while this feature describes the concrete outputs and the scripts that produce them.

**Depends on:** F-011 (Build-Time Automation Framework) — the token generator pipeline is the set of scripts within the F-011 automation infrastructure.
...
**F-201 · Theme Switching — Scaffolded, Not Built** `packages/tokens/src/themes/` contains three files — `dark.ts`, `light.ts`, `high-contrast.ts` — and all three are unimplemented stubs that throw `"not implemented"` if called; none is imported or called anywhere in the codebase. This is not a gap in the shipped visual identity: the actual forest-and-gold theme is hardcoded directly into `semantic/colors.ts` (which reads from `primitives/colors.ts`), with no theme-switching layer in between. What doesn't exist yet is any way to swap that for a light mode or a high-contrast accessibility mode — the `themes/` folder is scaffolding for that future feature, not a broken version of the current one. Worth knowing before a principal demo: if anyone asks "does it have a light mode toggle," the honest answer is no, not yet, and the folder that would hold it is empty.

_Numbered out of sequence — appended at the end of the registry per the stable-number rule, since this was identified well after F-012–F-020 were assigned._
...
**Correction — "still 13" is true in the contracts layer only, not in the running site:** `results` has a full `PageRegistry` entry (`registry/page-registry/results.ts`) and is a member of `PAGE_KEY_VALUES`, but there is no `apps/web/src/app/[locale]/results/page.tsx` — the route doesn't exist. `packages/config/src/paths.ts` already documents this precisely, in its own comment: _"'results' intentionally excluded: the page key exists in the registry (F-052) but apps/web has no results/page.tsx yet. Add it here once shipped."_ `PUBLIC_PAGE_KEYS` (which the sitemap, F-105, and the public nav are built from) is 12 keys, not 13 — `results` is the one omitted. So depending which layer you're counting from, the honest answer to "how many pages does the site have" is either 13 (contracts) or 12-live-plus-4-standalone (see next paragraph) — not a single clean number, and this entry previously implied there was one.

**Separately, four routes exist in `apps/web` that never went through this registry at all:** `/search`, `/achievements`, `/alumni`, and `/archive` (the last of these is the page the Proposal and prior documentation call "Digital Archive" — the actual folder and route segment is `archive`, not `digital-archive`; there is no `digital-archive` directory anywhere in the codebase). These four have `page.tsx` files and are real, but they don't have a `PageRegistry` entry, aren't in `PAGE_KEY_VALUES`, and don't source their content from `ContentEntry` the way the 12 live registry pages do. So "13 pages" was never really one system — it's 12 registry-driven pages, one registered-but-unrouted page (`results`), and 4 routed-but-unregistered pages, and no single feature entry in this registry said so until now.
...
**F-060 · Database Seed Script** Seeds `ContentEntry` rows for the pages that have been migrated onto the new foundation (currently: five pages and two global sections — see the correction below), idempotently upserted on the `(scope, sectionKey, locale)` unique key. **This entry undercounted the actual scope:** as of this revision, `seed.ts` runs `seedGlobals()` (Footer, Navigation) then `seedPages()` — which seeds five pages (About, Academics, Administration, Contact, Facilities), not just About. Eight of the thirteen registry pages (Admissions, Events, Extracurriculars, Gallery, Home, News, Results, Societies) still have no seed file and no `ContentEntry` rows, so they render empty in a fresh database until seeded. Seeding the initial admin user, default roles, and permissions is deferred along with the `User` model (F-057) and will be added back to this script once Google Workspace OAuth access control exists.
...
**F-200 · Transport Layer for Non-tRPC API Calls (`packages/transport`)** A second, separate package from `packages/api` — not previously declared anywhere in this registry — for the calls that don't go through tRPC: Resend, R2, reCAPTCHA, and any other plain REST endpoint. One HTTP client (`api.get/post/put/patch/delete`), one error shape (`ApiError`, with `normalizeError` and `codeFromStatus` for consistent handling), and shared helpers for headers (`createAuthHeaders`, `createCacheHeaders`), retry, timeout, pagination (cursor and offset), and serialization (JSON, form-data, search params) — so third-party integrations don't each reinvent `fetch`, a retry loop, and an `AbortController`. The package's own README is explicit about its boundary: it is deliberately not a replacement for tRPC (`@nexus/api`, F-064), has no Next.js/React/hooks/query-library dependency, no Prisma or database access, and holds no app-specific functions like a `getNews()` — those belong in a service layer built on top of it, not inside it.

_F-195 and F-200 are both numbered out of sequence — appended at the end of the registry per the stable-number rule (see How to Read This Document), rather than renumbering Group 9, since each was identified after the preceding number range was already assigned._
...
**F-105 · Dynamic Sitemap** `sitemap.ts` using Next.js conventions generates a sitemap. Submitted to Google Search Console and Bing Webmaster Tools. **The "and all dynamic pages" half of this claim doesn't hold up against the actual code:** `sitemap.ts` only iterates `PUBLIC_PAGE_KEYS` (the 12 static top-level pages, see F-052's correction) across the three locales — no news article, no society, no gallery album is individually listed. The file's own comment still says _"news/[slug] articles aren't included — news/[slug]/page.tsx (F-130) isn't built yet"_ — but that comment is now stale too: `apps/web/src/app/[locale]/news/[slug]/page.tsx` exists. So the sitemap needs updating on two counts — add per-article/per-album entries, and fix the comment that says the blocker for doing so still exists when it doesn't. Worth noting the same file already fixed a related, similar drift once: its own comment records that the previous hand-typed route list had silently drifted and was missing `extracurriculars`, `facilities`, and `gallery` before it was rewritten to source from `PUBLIC_PAGE_KEYS` directly — the same class of problem this correction is about, recurring one level up the stack.

**F-106 · robots.txt** Allows indexing of all public pages. Disallows the admin panel and API routes. **This is currently broken in code, not just undocumented:** the file implementing it is saved as `apps/web/src/app/robot.ts` (missing the ‘s’). Next.js's metadata-route convention requires the exact filename `robots.ts` to recognize and serve a file at `/robots.txt` — `robot.ts` is not a convention Next.js recognizes, so it's dead code, and there is no `public/robots.txt` static fallback either. The file's own header comment says `// apps/web/src/app/robots.ts` and describes exactly the behaviour this entry promises — the code was clearly written to be the real thing, it just isn't saved under the name that makes Next.js pick it up. Right now, in this snapshot, the site most likely serves no robots.txt at all. One-character fix (`robot.ts` → `robots.ts`), flagged here because it's the kind of thing that's invisible until a crawler or Search Console notices.
...
**F-116 · Multi-Stage Dockerfiles** Dockerfiles for `apps/web` and `apps/admin` use multi-stage builds: install and build stage, then a minimal runtime stage with only the compiled output. Production images are small, fast to pull, and contain no build tools. **A third Dockerfile exists at `apps/migrate/Dockerfile` and is not accounted for here or anywhere else in this registry.** Its build stage looks correct for a migration runner — it runs `pnpm --filter @nexus/db exec prisma generate` — but its build order and runtime stage are otherwise identical to an admin-panel build (`@nexus/tokens` → `@nexus/contracts` → `@nexus/env` → `@nexus/config` → `@nexus/api` → `@nexus/admin`, then a runner stage that copies `apps/admin/.next/standalone` and runs `node server.js`). There is no `prisma migrate deploy` (or equivalent) anywhere in the file, and no other file exists under `apps/migrate/` — no `package.json`, no source. This reads like the admin Dockerfile was copied here as a starting point for a dedicated migration container and never finished; as written, `apps/migrate` builds and runs the admin panel, not a migration. Flagging rather than guessing further — worth a direct look before this goes anywhere near a principal-facing deck.
...
**Group-wide correction, and the single most important finding in this revision:** this group was written as if every page below is a working, built page. Checking each `page.tsx` against its description shows a hard split. Six routes are genuinely built, with real imports, real block components, and a real content-fetching call: **Home** (partially — see F-141), **About** (F-142), **Academics** (F-159), **Administration** (F-160), **Contact** (F-153), **Facilities** (F-149). Every other route in this group — News (F-143, F-144), Events (F-145, F-146), Societies (F-147, F-148), Admissions (F-150), Gallery (F-151, F-152), Extracurriculars (F-161), and the four routes covered in F-154–F-157 — is a `page.tsx` file containing only a handful of comment lines describing the intended page, with no component, no export, nothing that renders. This isn't a surprise once F-057 and F-069 are read alongside this group: none of those pages' backing Prisma models exist yet, and none of their tRPC routers are implemented or mounted — there's genuinely no data for them to render. What was missing wasn't the reason, it was this group saying so. Anyone opening this document to understand what's demo-ready needs to know that's five or six pages out of twenty, not the twenty this group implied.

**F-141 · Home Page (designed, not built)** The intended page, and what's registered in the Page Registry: Hero with CrestAnimation, headline, and call to action. Statistics Strip with animated counters. Principal's Message. Latest News (three most recent published articles). Upcoming Events (next three). Quick Access Portal (links to Admissions, Societies, Gallery). Announcement Banner when active. Section order is fixed by this page's entry in the Page Registry (F-052) — code, not a runtime-configurable setting. **None of this is what actually renders.** `apps/web/src/app/[locale]/page.tsx` is a bare client scaffold: an `<h1>Welcome to Nexus</h1>` and five plain `ButtonLink`s to About, Academics, Administration, Contact, and Facilities — no Hero, no CrestAnimation, no stats, no Principal's Message, no news, no events, no Quick Access Portal, no Announcement Banner. Two small bugs sit in this scaffold too, worth fixing whenever it's rebuilt rather than papered over: the button meant for Contact is labelled "About" (a duplicate of the real About button), and the Facilities link points to `./facilites` (missing an i). There's also a commented-out `throw new Error('Testing error page')` line still in the file — a leftover from testing F-030's error state, harmless but worth removing when this page is rebuilt for real. This is the single highest-visibility gap in the entire registry: it's the first thing anyone sees, including the principal.
...
**F-143 · News Listing Page (stub only)** The intended page: all published news articles with category filter, pagination, and search, server-rendered with static generation for each category. **`apps/web/src/app/[locale]/news/page.tsx` is a 194-character comment-only file** — no component, no data fetch. Blocked on the `News` Prisma model and `newsRouter`, both listed as not-yet-built in F-057 and F-069.

**F-144 · News Article Page (stub only)** The intended page: individual article with rich text rendering via RichTextRenderer, author attribution, publication date, related articles, social sharing via ShareSheet. **`apps/web/src/app/[locale]/news/[slug]/page.tsx` is a 175-character comment-only file** — no component. This is also why F-105's sitemap correction is right to leave news articles out for now: there's no article data to enumerate, comment-stub or not.

**F-145 · Events / School Calendar Listing Page (designed, not built)** The design, unchanged from the previous revision: one page, two views, over a single dataset — every `CalendarEntry` (F-198), filterable by category and month. The calendar view renders every entry; the list view renders only entries with a linked `EventDetail` as a full `EventCard`. **The actual file, `apps/web/src/app/[locale]/events/page.tsx`, is a 156-character comment-only stub** describing an older, pre-F-198 version of this page ("Calendar view + list view... Upcoming vs past events distinguished") — no component exists to build against the newer two-tier design at all yet. F-198 is a design to build toward, not a description of something already running.

**F-146 · Event Detail Page (designed, not built)** Full event description, date, time, venue, category, optional registration link, map embed for venue location — rendered only for a `CalendarEntry` with a linked `EventDetail` (F-198). **`apps/web/src/app/[locale]/events/[slug]/page.tsx` is a 176-character comment-only stub**, again describing the pre-F-198 standalone-`Event` model rather than the calendar-linked design. No component exists yet.

**F-147 · Societies Hub (stub only)** The intended page: all societies with category filter, each shown as a SocietyCard. **`apps/web/src/app/[locale]/societies/page.tsx` is a 127-character comment-only file** — no component. Blocked on the `Society` Prisma model and `societiesRouter` (F-057, F-069).

**F-148 · Society Detail Page (stub only)** The intended page: banner, description, advisor StaffCard, founding year, member count, recent events, gallery preview. **`apps/web/src/app/[locale]/societies/[slug]/page.tsx` is a 121-character comment-only file** — no component.
...
**F-150 · Admissions Page (stub only)** The intended page: admissions process via ProcessSteps, key dates via AdmissionsKeyDatesTimeline, requirements, ContactForm for admissions enquiries. **`apps/web/src/app/[locale]/admissions/page.tsx` is a 146-character comment-only file** — no component, no data fetch. The "sourced from Events table" detail was already inaccurate before the stub question even comes up: there is no `Event` table (F-057 lists it as deferred), and admissions key dates are their own domain schema under `domains/admissions/` (F-051), unrelated to calendar events — this page's own header comment repeats the same wrong claim, so it didn't originate as a documentation-only error.

**F-151 · Gallery Listing Page (stub only)** The intended page: albums sorted by year, GalleryAlbumCard for each, year-based filtering. **`apps/web/src/app/[locale]/gallery/page.tsx` is a 108-character comment-only file** — no component. Blocked on the `GalleryAlbum`/`GalleryPhoto` Prisma models and `galleryRouter` (F-057, F-069).

**F-152 · Gallery Album Page (stub only)** The intended page: photo grid with Lightbox for full-screen viewing, all images via `next/image`, alt text on every photo. **`apps/web/src/app/[locale]/gallery/[albumSlug]/page.tsx` is a 113-character comment-only file** — no component.
...
**F-154 · Alumni Directory (route exists, page does not)** The intended feature: searchable by graduation year, profession, country. Shows name, year, position, quote — no private contact information. Submission form for new profiles enters an admin approval queue. **What actually exists at `apps/web/src/app/[locale]/alumni/page.tsx` is a 164-character file containing only three comment lines describing this feature — no component, no export, nothing that renders.** This is a routed, blank page, not a built one. The comment's own cross-reference (`F-140`) is also stale — that's the Testing group now, not this feature.

**F-155 · Digital Archive (route exists, page does not)** The intended feature: historical photographs, old annual magazines (PDFs with searchable metadata), prize-giving records, prefect lists by year, browsable by year and searchable by full-text. **What actually exists at `apps/web/src/app/[locale]/archive/page.tsx` is a 185-character file containing only three comment lines describing this feature — no component, no export, nothing that renders.** Also worth fixing while here: the route segment is `archive`, not `digital-archive` — there is no `digital-archive` folder anywhere in the codebase, despite that spelling appearing in the Project Proposal, the sitemap example in prior documentation, and this feature's own name. The comment's cross-reference (`F-141`) is stale too — that's the Home Page now.

**F-156 · Achievement Database (route exists, page does not)** The intended feature: academic achievements (A/L results, university admissions), sports (tournament wins, national athletes), arts and cultural, competition results, filterable by year, category, student name. **What actually exists at `apps/web/src/app/[locale]/achievements/page.tsx` is a 147-character file containing only three comment lines describing this feature — no component, no export, nothing that renders.** The comment's cross-reference (`F-142`) is stale too — that's the About Page now.

**F-157 · Unified Search (route exists, page does not)** The intended feature: single search box accessible from every page via Navigation, querying across all content types simultaneously via PostgreSQL full-text search supporting Sinhala, Tamil, and English, results grouped by type and ranked by relevance. **What actually exists at `apps/web/src/app/[locale]/search/page.tsx` is a 164-character file containing only three comment lines describing this feature — no component, no export, nothing that renders.** The comment's cross-reference (`F-143`) is stale too — that's the News Listing Page now.

**A pattern across all four of F-154–F-157:** each has a real Next.js route folder and a `page.tsx` file, which is exactly the kind of thing that makes a directory listing or a quick `apps/web/src/app` skim look like the page is built. Every one of the four files is a comment-only stub. None of them source from a Prisma model — fitting, since none of the backing models (`AlumniProfile`, `MediaAsset`/archive equivalent) exist yet either (F-057). Treat these four as fully unbuilt, not partially built, until a real component lands in each file.

**F-158 · Cookie Consent Banner (component built, not wired in)** The intended behaviour: CookieConsentBanner shown on first visit, persisted to localStorage, no analytics cookies set before consent. The component itself is real and correctly built (F-030/F-031). **It is never mounted on the public site.** `apps/web/src/app/[locale]/layout.tsx` — the actual root layout every page renders through — only renders `NextIntlClientProvider`, `BackToTopButton`, and `Footer`. No `CookieConsentBanner`, and no site navigation/header of any kind either — there is currently no `Navigation` component (F-033) in the real layout at all. The only place `CookieConsentBanner` is actually rendered anywhere in the codebase is inside the admin design-system showcase (F-020), as a demo. As written today, the public site sets no consent banner and collects no consent — worth knowing given F-158's own justification is GDPR compliance for an international diaspora.
...
**F-161 · Extracurriculars Page (stub only)** The intended page: sports teams and co-curricular activities via ExtracurricularCard, distinct from Societies, plus an ExtracurricularsJoinCTA section. **`apps/web/src/app/[locale]/extracurriculars/page.tsx` is a 182-character comment-only file** — no component.
...
| Group                                                            | Range                       | Count   |     |
| ---------------------------------------------------------------- | --------------------------- | ------- | --- |
| Monorepo & Developer Tooling                                     | F-001 – F-011, F-199        | 12      |     |
| Design System Tokens                                             | F-012 – F-020, F-201        | 10      |     |
...
| API Layer                                                        | F-064 – F-072, F-195, F-200 | 11      |     |
