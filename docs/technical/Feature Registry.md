...
**M1a update:** `system/rbac/matrix.ts` now implements the concrete grants behind `RoleEnum` (see F-075) — `hasPermission()`/`checkPermission()`, not just the shape. The Prisma `User` model (F-057) is real as of this milestone too, matching `UserSchema`'s shape closely enough to authenticate against, though the two were not mechanically reconciled field-by-field.
...
**Depends on:** F-047 (block library — `EventCardSchema`'s projection is consumed the same way any block-rendered card is), F-048 (Editorial Content Schemas — this feature supersedes the Events sub-entry described there). Implementing this means relating `CalendarEntry` to `EventDetail` (renamed from the standalone `EventSchema`) by id, updating the Events Module (F-166) to present the single two-branch creation form, and updating the Events Listing Page (F-145) and Event Detail Page (F-146) to read accordingly.
...
**M1a update:** `User` is no longer deferred — added along with `Account`, `Session`, `VerificationToken` (Auth.js adapter shape) and `BackupCode` (F-080), ahead of the rest of this list, since Task 6.3/6.5 needed it. `News`, `Staff`, `Society`, `Event`, `GalleryAlbum`, `GalleryPhoto`, `Achievement`, `AlumniProfile`, `MediaAsset`, `AuditLog`, and `NotificationQueue` remain deferred exactly as described above.
...
**F-150 · Admissions Page** Admissions process via ProcessSteps. Key dates via AdmissionsKeyDatesTimeline, sourced from the admissions domain schema (F-051), not from calendar events. Requirements. ContactForm for admissions enquiries.
...
**F-190 · Operational Runbook** `docs/operations/Operational Runbook.md` — step-by-step instructions for every operation a maintainer might need: deploy a change, roll back a bad deployment, restore from backup, add a new admin user, add a new language, debug a failing API route, renew a TLS certificate, scale the server.

**F-191 · Disaster Recovery Plan** `docs/operations/Disaster Recovery Plan.md` — documented recovery procedures for every failure mode: server failure, database corruption, accidental content deletion, domain loss, GitHub repository loss, R2 storage failure. Includes estimated recovery times and contact responsibilities. Every procedure has been tested before handover.
...
**This revision:** fixed two stale file paths — F-190 pointed to `docs/operations/Runbook.md` (deleted, superseded by `Operational Runbook.md`) and F-191 pointed to `docs/operations/Disaster Recovery.md` (deleted, superseded by `Disaster Recovery Plan.md`). Independently verified the Summary Table's total against the actual F-001–F-200 numbering in this document — it's accurate; 200 features, no gaps, no double-counting.
