...
1. **Draft** – Author writes in Sanity, saves as draft.
2. **Review** – Editorial team member reviews.
3. **Fact‑check** – Verify dates, names, quotes.
4. **Approve** – Head of editorial or administration approves.
5. **Schedule** – Set `publishedAt` (future dates allowed).
6. **Publish** – Automatically published at scheduled time.

**Slug:** Auto‑generated from title, can be overridden. Must be unique.

### Society Pages

1. **Proposal** – Student committee creates draft.
...
- Changes require photo update and title/portfolio verification.
- Published immediately after review.

### Gallery Albums

- Media unit uploads photos (max 100 per album).
- Photos must be optimised (WebP, ≤1MB).
...
**Deletion:** Content is never hard‑deleted. Instead, set `status: "archived"` in CMS. Archived content is not shown in listings but can be accessed via direct URL (for news) or admin panel.
...
- Sanity automatically tracks revisions.
- Every change must have a comment explaining the reason (required field in Sanity if `editedBy` not equal to original author).
...
2. Complete a Sanity CMS tutorial (recorded).
