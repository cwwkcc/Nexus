...
## Immediate Codebase Issues to Fix

These exist in the current repo and must be resolved before Phase 1 begins:

1. **`apps/web/tailwind.config.js`** — `font-heading: 'DM Serif Display'` must be removed. DM Serif Display was formally removed from the design system.
    
2. **`apps/web/src/app/global.css`** — `--font-heading` CSS variable must be removed or replaced with `--font-display`.
    
3. **`apps/web/tailwind.config.js`** — Color tokens do not fully match the finalized token system. Needs complete replacement with the token map from `_design-system-foundations.md`.
    
4. **`apps/web/tailwind.config.js`** — Missing tokens: interactive gold states, semantic colors, overlay values, elevation shadows, motion durations, easing values.
    
5. **`packages/ui/src/components/OrbitalPanel.tsx`** — References `gold` and `gold-light` class names that will change when Tailwind config is updated.
...
Everything needed for launch is already in `package.json`. No additional packages required for Phases 1–19.
...
|`next-intl`|✓ installed|
|`@sanity/client`, `@sanity/image-url`|✓ installed|
|`@aws-sdk/client-s3`, `@aws-sdk/s3-request-presigner`|✓ installed|
|`@prisma/client`, `prisma`|✓ installed|
|`@trpc/client`, `@trpc/server`, `@trpc/next`, `@trpc/react-query`|✓ installed|
