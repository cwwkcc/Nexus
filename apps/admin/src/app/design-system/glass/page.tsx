  ...
  return (
    <div>
      ...
      <p className="font-body text-body text-text-muted mb-space-4">
        Translucent surface tokens for the "forest mist" aesthetic. These are
        ...
        — use them with{' '}
        <code className="font-mono text-code bg-surface-default px-space-2 py-space-1 rounded-sm">
          style={'{{ backgroundColor: "var(--glass-surface-light)" }}'}
        </code>
        , or add a utility class in{' '}
        <code className="font-mono text-code bg-surface-default px-space-2 py-space-1 rounded-sm">
          global.css
        </code>
        .
          ...
          {glassSurfaces.map((s) => (
            <div key={s.name} className="flex flex-col gap-space-3">
              ...
              <div
                ...
                style={{
                  background:
                    'linear-gradient(135deg, var(--color-green-base) 0%, var(--color-gold-base) 100%)',
                }}
              >
                ...
                >
                  <span className="font-body text-label tracking-label text-text-inverse">
                    {s.label}
                  </span>
                  <span
                    className="font-mono text-caption text-text-inverse"
                    style={{ opacity: 0.8 }}
                  >
                    {s.value}
                  </span>
      ...
      <div>
        ...
        <div className="flex flex-col gap-space-3">
          {allTokens.map(({ name, value }) => (
            <div
              key={name}
              className="flex items-start gap-space-4 p-space-4 bg-surface-elevated border border-border-light rounded-md"
            >
              <span className="w-size-68 shrink-0 font-mono text-label text-text-primary">
                --{name}
              </span>
              <span className="font-mono text-caption text-text-muted break-all">
                {value}
              </span>
            </div>
          ))}
