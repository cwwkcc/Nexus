...
const foundations = [
  ...
  },
  {
    ...
    label: 'Typography',
    description:
      'Cormorant Garamond, Inter, Sinhala fonts, and the full type scale.',
  },
  {
  ...
  },
  {
    ...
    label: 'Sizing',
    description:
      'Width and height tokens – component, block, layout, and viewport scales.',
  },
  {
    href: '/design-system/motion',
    label: 'Motion',
    description:
      'Duration and easing tokens including ceremonial and ember curves.',
  },
  {
  ...
  return (
    <div>
      <p className="font-body text-body text-text-muted mb-space-8 max-w-prose">
        Every visual decision in Nexus flows from design tokens — the single
        source of truth shared across the public site and the admin panel.
      </p>
      ...
      <div className="mb-space-10 p-space-6 border border-gold-base rounded-md bg-surface-elevated flex items-center justify-between gap-space-6">
        ...
        <Link
          ...
          className="shrink-0 font-body text-label tracking-label text-gold-base hover:text-gold-hover transition-colors duration-fast"
        >
          Browse →
        </Link>
      </div>
      ...
      {/* Foundations grid */}
      <h3 className="font-display text-h3 mb-space-6 border-b border-border-light pb-space-2">
        Foundations
      </h3>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-space-4">
        {foundations.map((f) => (
          <Link
            key={f.href}
            href={f.href}
            className="group block p-space-5 border border-border-light rounded-md bg-surface-elevated hover:border-gold-base transition-all duration-fast"
          >
            <h4 className="font-display text-h4 mb-space-1 group-hover:text-gold-base transition-colors duration-fast">
              {f.label}
            </h4>
            <p className="font-body text-body-sm text-text-muted">
              {f.description}
