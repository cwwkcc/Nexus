...
const colorGroups = [
  ...
  {
    name: 'Surfaces',
    tokens: ['surface-elevated', 'surface-base', 'surface-default', 'surface-deep', 'surface-hover', 'surface-active', 'surface-disabled', 'surface-inverse'],
  },
  {
    name: 'Text',
...
export default function ColorsPage() {
  ...
  return (
    <div>
      ...
      <p className="font-body text-body text-text-muted mb-space-8">All colors are referenced via tokens – no raw hex values in components.</p>
        ...
        <div key={group.name} className="mb-space-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-space-4">
            {group.tokens.map((token) => (
              <div key={token} className="border border-border-light rounded-md overflow-hidden">
                <div className="h-size-24 w-full" style={{ backgroundColor: `var(--color-${token})` }} />
                <div className="p-space-3 bg-surface-elevated">
                  <div className="font-mono text-caption font-medium">{token}</div>
                  <div className="font-mono text-caption text-text-muted">{cssValues[token] ?? ''}</div>
                </div>
              </div>
            ))}
          </div>
