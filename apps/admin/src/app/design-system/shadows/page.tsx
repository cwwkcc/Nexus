    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-space-6">
        {shadowTokens.map((token) => (
          <div
            key={token}
            className="p-space-6 bg-surface-elevated rounded-md text-center"
            style={{ boxShadow: `var(--shadow-${token})` }}
          >
            <div className="font-mono text-label font-medium mb-space-2">
              {token}
            </div>
            <div className="font-mono text-caption text-text-muted break-all">
              {resolvedValues[token] ?? `var(--shadow-${token})`}
            </div>
          </div>
        ))}
      </div>
